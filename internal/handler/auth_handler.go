...
func (h *AuthHandler) Register(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&input); err != nil {
		writeJSON(
			...
			map[string]string{"error": "virheellinen JSON-syöte"},
		)
		return
	}
	defer r.Body.Close()
...
func (h *AuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		writeJSON(
			...
			map[string]string{"error": "sisäinen palvelinvirhe"},
		)
		return
	}
...
// Me käsittelee GET /api/v1/auth/me -pyynnöt.
//
// Endpoint on suojattu AuthMiddlewarella.
// Middleware lisää autentikoidun käyttäjän ID:n requestin contextiin,
// josta se luetaan tässä handlerissa.
func (h *AuthHandler) Me(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		writeJSON(
			w,
			http.StatusUnauthorized,
			map[string]string{"error": "autentikaatio puuttuu"},
		)
		return
	}
	...
	if err != nil {
		writeJSON(
			w,
			http.StatusNotFound,
			map[string]string{"error": "käyttäjää ei löydy"},
		)
		return
	}
