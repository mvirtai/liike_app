...
func TestWorkoutRepository_CRUD(t *testing.T) {
	...
	if found.ExerciseType == nil || found.ExerciseType.Name != "Kyykky" {
		t.Errorf("odotettiin lajia 'Kyykky', saatiin %v", found.ExerciseType.Name)
	}

	// 4. Listataan käyttäjän suoritukset
	list, err := repo.ListWorkoutsByUserID(ctx, user.ID, repository.WorkoutFilter{})
	if err != nil {
		t.Fatalf("ListWorkoutByUserID epäonnistui: %v", err)
	}
	if len(list) != 1 {
		t.Errorf("odotettiin 1 suoritus, saatiin %d", len(list))
	}

	// 5. Poistetaan suoritus
